...
function initClient() {
  ...
  if (!config.url || !config.anonKey || config.url.includes("PASTE_YOUR")) {
    setLoginStatus("Supabase is not configured yet. Add your project URL and anon key to beadlight/supabase-config.js.", true);
    return null;
  }
...
async function sendMagicLink() {
  ...
  if (!email) return setLoginStatus("Enter your email address first.", true);

  setLoginStatus("Sending magic link…");
  const { error } = await client.auth.signInWithOtp({
    email,
    options: { emailRedirectTo: `${window.location.origin}/beadlight/admin/` }
  });
...
function renderEditor() {
  editor.innerHTML = items.length ? items.map(renderEditorItem).join("") : `<div class="empty-state">No roadmap items yet.</div>`;

  editor.querySelectorAll("[data-action='save']").forEach(btn => btn.addEventListener("click", () => saveItem(btn.dataset.id)));
  editor.querySelectorAll("[data-action='delete']").forEach(btn => btn.addEventListener("click", () => deleteItem(btn.dataset.id)));
}

function renderEditorItem(item) {
  const statusOptions = STATUSES.map(([value, label]) => `<option value="${value}" ${item.status === value ? "selected" : ""}>${label}</option>`).join("");
  return `
    <article class="editor-card" data-id="${item.id}">
      <div class="admin-grid">
        ...
        <label>Tag<input data-field="tag" value="${escapeAttr(item.tag || "")}" placeholder="e.g. Android, Audio, UI"></label>
        <label>Priority<input data-field="priority" value="${escapeAttr(item.priority || "")}" placeholder="High / Medium / Low"></label>
        <label>Sort order<input data-field="sort_order" type="number" value="${Number(item.sort_order || 0)}"></label>
        <label>Visible<select data-field="is_public"><option value="true" ${item.is_public ? "selected" : ""}>Public</option><option value="false" ${!item.is_public ? "selected" : ""}>Hidden</option></select></label>
        <label class="wide">Summary<textarea data-field="summary" rows="3">${escapeHtml(item.summary || "")}</textarea></label>
      </div>
...
async function saveItem(id) {
  ...
  if (!values.title) return setAdminStatus("Title is required.", true);

  setAdminStatus("Saving…");
  const { error } = await client.from("roadmap_items").update(values).eq("id", id);
...
async function deleteItem(id) {
  ...
  const { error } = await client.from("roadmap_items").delete().eq("id", id);
  if (error) return setAdminStatus(error.message, true);
  await loadItems();
